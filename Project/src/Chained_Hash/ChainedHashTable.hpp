...
public:
    ...
    bool add(const Key& k, const Value& v) {
        ...
        for(auto& p : m_table[slot]) {
            ...
            if(p.first == k) {
                ...
                return false; // indica que uma atualização foi feita, mas não uma inserção
            }
        }
        ...
        return true; // indica que uma nova inserção foi feita
    }
    ...
    Value& operator[](const Key& k) {
        std::cout << "--- DEBUG: operator[] da ChainedHashTable foi chamado! ---" << std::endl;
        if(load_factor() >= m_max_load_factor) {
            rehash(2 * m_table_size);
        }
        ...
        // Lógica de colisão adicionada
        if (!m_table[slot].empty()) { // <-- ADICIONADO
            collisions++;
        }
    ...
    long long get_comparisons() const { return comparisons; } // Função que retorna o número de comparações
    long long get_collisions() const { return collisions; }   // Função que retorna o número de colisões
};
